...
function Stream() {
  ...
  return (
    ...
    className="min-h-screen bg-radial-[at_50%_50%] 
             from-[#1E1B4B] via-[#0B0F1A] to-[#020617] 
             px-2 md:px-16 py-3"
    >
      <Navbar />
      ...
      <section className="md:max-w-5/6 mb-10 mx-auto">
        <div className="mt-12 bg-white/5 backdrop-blur-lg p-10 rounded-3xl border border-white/10">
          ...
          <p className="mt-3 max-w-3xl text-sm md:text-base text-white/70">
            Real-time money streaming on Arbitrum. Create programmable flows of
        ...
        <div className="grid gap-10 md:grid-cols-2">
          ...
          <div className="flex mt-2 mx-auto w-fit">
            <button  onClick={()=> setStream("single")} className={` ${stream === "single" ? 'bg-[#7C3AED]' : '' }  border  shadow-2xl  text-white/50 md:py-3 py-2 md:px-6 px-4  border-white/10 rounded-l-full`}>
                SINGLE STREAM
            </button>
            <button onClick={()=> setStream("batch")}  className={` ${stream === "batch" ? 'bg-[#7C3AED]' : '' }  border  shadow-2xl  text-white/50 md:py-3 py-2 md:px-6 px-4  border-white/10 rounded-r-full`}>
                MULTIPLE STREAM
            </button>
          </div>
          {
              ...
              <div className="col-span-2 ">
                <button
                  ...
                  className={`
                    ...
                    : 'bg-[#7C3AED] text-black hover:bg-purple-400'}
                    w-full cursor-pointer px-8 py-3 rounded-xl bg-[#7C3AED] text-black font-medium text-lg hover:bg-purple-400 transition`}
                >
                  Create Stream
                </button>
              </div>
             ...
             >
                ...
                <div className="flex justify-end">
                  <button
                    ...
                    className="px-6 py-3 rounded-full bg-[#7C3AED] text-white/70 font-medium hover:bg-purple-400 transition"
                  >
                    + ADD RECIPIENT
                  </button>
                </div>
          ...
          <div className="mt-10 h-fit ">
            <h1 className="text-3xl pb-6 text-center font-bold text-[#7C3AED]">
              STREAM PREVIEW
            </h1>
            <div className=" transform transition duration-500 hover:-translate-y-1  text-white/70 bg-white/5 text-center backdrop-blur-lg p-10 rounded-3xl border border-white/10">
              ...
              <p className="text-2xl pb-2 font-bold text-[#7C3AED]">$ <span>{amountEth}</span> USDT</p>
              <p className="pb-2"> 
      ...
      <div className="flex fixed bottom-0 left-0 right-0 z-50 bg-linear-to-r from-[#0C1220] to-[#0F1625] py-4 md:hidden justify-center  space-x-7 text-[#ffffff] font-light">
          <NavLink to="/" style={({ isActive })=>({
            color: isActive ? "#3B82F6 " : '',})}  className={` hover:text-[#7C3AED] cursor-pointer transition flex `}>  <span>Home</span></NavLink>
          <NavLink to="/stream"  style={({ isActive })=>({color: isActive ? "#3B82F6" : ''})}  className={`hover:text-[#7C3AED] cursor-pointer transition flex `}> <span>Stream</span></NavLink>
          <NavLink to="/withdraw"  style={({ isActive })=>({color: isActive ? "#3B82F6" : ''})}  className={`hover:text-[#7C3AED] cursor-pointer transition flex `}>  <span>Withdraw</span> </NavLink>
          <NavLink to="/history"  style={({ isActive })=>({color: isActive ? "#3B82F6" : ''})}  className={`hover:text-[#7C3AED] cursor-pointer transition flex `}> <span>History</span></NavLink>
      </div>
    </motion.div>
  );
}
